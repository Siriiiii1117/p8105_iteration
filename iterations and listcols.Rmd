...
```{r}
output2 = map(list_normals, mean_and_sd)
output2
```
...
map_dfr(list_normals, mean_and_sd, .id = "sample") # create id

map_dbl(list_normals, median) # know if the output is single numbers 
```
...
```{r}
map(1:3, nsduh_import, html = nsduh_html) # because nsduh_import has two arguments
```
